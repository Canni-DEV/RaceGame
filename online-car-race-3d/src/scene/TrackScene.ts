...
export class TrackScene {
  ...
  setFollowPlayer(playerId: string | null = this.playerId): void {
  ...
  private syncCars(states: CarState[]): void {
    ...
    for (const [playerId, entity] of this.cars.entries()) {
      if (!active.has(playerId)) {
        ...
        if (this.followPlayerId === playerId) {
  ...
  private updateCameraFollow(): void {
    ...
    const entity = this.cars.get(this.followPlayerId)
    const object = entity?.getObject() ?? null
